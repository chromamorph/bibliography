...
These functions culminate in the function sort-by, and
were written when I was new to Lisp. As such, there
are faster, more robust, in-built alternatives for
...
(defun break-fixed-with-sort-by-col
  ...
  (let ((a-partition
	 (rows-with-fixed-same-as-1st-row
	  fixed a-list)))
    (if (null a-list) ()
      (append (sort-by-col col a-partition direction)
...
(defun max-nth-argmax (col a-list)
  (let* ((*max-nth* (max-nth col a-list))
	 (*argmax* (index-item-1st-occurs *max-nth*
					  a-list)))
    (list *max-nth* *argmax*)))
...
(defun min-argmin (a-list)
  (let* ((*min* (min-item a-list))
	 (*argmin* (index-item-1st-occurs *min*
					  a-list)))
    (list *min* *argmin*)))
...
(defun min-nth-argmin (col a-list)
  (let* ((*min-nth* (min-nth col a-list))
	 (*argmin* (index-item-1st-occurs *min-nth*
					  a-list)))
    (list *min-nth* *argmin*)))
...
the list is returned. This has a bearing on higher-
level functions. |#
