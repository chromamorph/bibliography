...
%   transposition invariant content-based music retrieval. In Holger H.
%   Hoos and David Bainbridge (Eds.), Proceedings of the International
%   Symposium on Music Information Retrieval (Baltimore, MD: International
%   Society for Music Information Retrieval, 2003), 193-199.

% INPUT
%  P is a pattern.
%  D is a point set. It is assumed that P and D are in lexicographic order.
%   Otherwise
%   D = sortrows(D);
...
% translators = translatorsOfPatternInDataset(P, D);
...
if executeIteration
    ...
    while j <= n - m + 1
        ...
        while i <= m && J <= n
            ...
            if max(abs(q - (P(i,:) + f))) < 1e-10 % Test equality with
                                                  % tolerance for error.
                k=k+1;
                i=i+1;
                J=J+1;
