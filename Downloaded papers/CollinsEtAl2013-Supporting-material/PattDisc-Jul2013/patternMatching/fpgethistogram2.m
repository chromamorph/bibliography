function h = fpgethistogram2(D, P, fpType)

% Copyright Tom Collins 9/1/2013

% This function is an implementation of the fingerprinter of Arzt, Böck,
...
% time interval between two consecutive notes is less than this amount, the
% note pair will be skipped;
d = .01;
switch fpType
  case 'normal'
    trans_invar = 0;
...
res = 1;
% Time tolerance (default 1, 0.75 in paper). Part of a token is the time
% difference o2 - o1, or the time difference ratio r = (t3 - t2)/(t2 - t1).
% When lookup is performed with timeTolerance < 1, some inexactness in this
% differene (ratio) is permitted: a value of 0.75 permits a difference of
% .25 either way in the case of time difference, and 25% either way in the
% case of ratio.
timeTolerance = .85;
...
P = P - repmat([P(1, 1) P(1, 2:end)], size(P, 1), 1);
